...
def decrypt_reponse():  # assuming that data is on a file called payload.enc generated on the function get_data
    ...
    aux_list = []
    for element in out.split():
        aux_list.append(element.decode())
...
def create_config_ubuntu(response):
    ...
    if res['gateway']:
        ...
        subprocess.call('sudo cp /home/config/gw_ubuntu.sh /usr/local/bin/.', shell=True)
...
def final_settings_ubuntu():
    ...
    subprocess.call('sudo cp /home/config/mesh_init.sh /usr/local/bin/.', shell=True)
